#include "SupportSpotCalculator2.h"
...
//------------------------------- dtor ----------------------------------------
//-----------------------------------------------------------------------------
SupportSpotCalculator2::~SupportSpotCalculator2()
{
  delete m_pRegulator;
}
...
/*SupportSpotCalculator2::SupportSpotCalculator2()  {
	OutputDebugString("SSc2. default constructor\n");
}*/

void SupportSpotCalculator2::setParams(int           numX,
                                             int           numY,
											 SoccerTeam2*   team)  {
  ...
  for (int x=0; x<(numX/2)-1; ++x)
  {
    ...
    {      
      if (m_pTeam->Color() == SoccerTeam2::blue)
      {
...
//-----------------------------------------------------------------------------
Vector2D SupportSpotCalculator2::DetermineBestSupportingPosition()
{
  ...
  for (curSpot = m_Spots.begin(); curSpot != m_Spots.end(); ++curSpot)
  {
    ...
    //to this position?
	if(!m_pTeam->InControl())  {
		OutputDebugString("SupportSpot.determinebestspot: problem with the controlling player\n");
	}
...
//-----------------------------------------------------------------------------
Vector2D SupportSpotCalculator2::GetBestSupportingSpot()
{
...
//----------------------------------- Render ----------------------------------
//-----------------------------------------------------------------------------
void SupportSpotCalculator2::Render()const
{
    gdi->HollowBrush();
    gdi->GreyPen();
